...
from common.utils import poll_checkpoint_folder
    ...
    def eval(self) -> None:
        ...
        with TensorboardWriter(
            ...
            if os.path.isfile(self.config.EVAL_CKPT_PATH_DIR):
                logger.info(f"\n=======current_ckpt: {self.config.EVAL_CKPT_PATH_DIR}=======\n")
                # evaluate singe checkpoint
                self._eval_checkpoint(self.config.EVAL_CKPT_PATH_DIR, writer)
            else:
                # evaluate multiple checkpoints in order
                prev_ckpt_ind = self.config.EVAL_PREV_CKPT_ID + 1
                # logger.info(f"EVAL_PREV_CKPT_ID = {self.config.EVAL_PREV_CKPT_ID}")
                while True:
                    current_ckpt = None
