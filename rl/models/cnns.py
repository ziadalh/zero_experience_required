...
class SimpleCNN(nn.Module):
    ...
    def __init__(
        ...
        cnn_dims = np.array(input_shape[:2], dtype=np.float32)
        self._n_input_channels = input_shape[2]
    ...
    def forward(self, x):
        ...
        if self.output_reshape is not None:
            # keep batch dim and reshape the rest
            y = y.view(y.size(0), *self.output_reshape)
